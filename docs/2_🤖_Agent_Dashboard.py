...
def start_documentation_generation():
    ...
    **✅ Ready to Generate Documentation**
    ...
    💰 **Cost:** ~$0.02 per document (12x cheaper than Sonnet)  
    ...
    if st.button("🚀 Start Generation", type="primary", disabled=button_disabled):
        ...
        if not limiter.check_global_limit():
            ...
            st.info("""
            The system has generated 100 documents today (our $2 daily budget limit).
            ...
            try:
                ...
                with st.spinner("Generating documentation... This may take several minutes."):
                    ...
                    st.session_state.workflow_logs.append({
                        ...
                        "message": "Using claude-haiku-4-5-20251001 (~$0.02/doc)",
                        "level": "info"
                    })
...
def main():
    ...
    with st.sidebar:
        ...
        if 'rate_limiter' in st.session_state:
            ...
            budget_used = stats['total_generations_today'] * 0.02
            budget_remaining = stats['global_remaining'] * 0.02
            st.caption(f"Budget: ${budget_used:.2f} / $2.00 (${budget_remaining:.2f} remaining)")
    # ============================================================
